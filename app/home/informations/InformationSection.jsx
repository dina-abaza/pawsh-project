  ...
  return (
    <section className="py-16 px-6 md:px-12 bg-gray-50">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        {infoCards.map((card) => (
          <Link key={card.id} href={`/home/informations/${card.id}`}>
            <div
              className={`flex items-stretch rounded-2xl shadow-lg overflow-hidden ${card.bg} transition-transform duration-300 hover:-translate-y-1 cursor-pointer`}
            >
              {/* صورة الكارت */}
